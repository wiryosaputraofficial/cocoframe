...
const questionCatalog: readonly (CocoQaQuestion & { readonly minimumMode: CocoQaMode; readonly applies?: (qa: CocoQa) => boolean })[] = [
  ...
  question("design-evidence", "visual", "Which measurements and manual reviews must prove each Product Design Quality principle?", "Turns alignment, spacing, color, iconography, overflow, contrast, and fidelity expectations into traceable evidence.", "Map each design case to sanitized screenshots, computed measurements, accessibility results, or reviewer evidence.", "standard", hasDesignCases),
...
export function createCocoQa(options: CreateCocoQaOptions): CocoQa {
  ...
  const cases: CocoQaCase[] = [
    ...
    { id: "framework-target-reachability", title: "Every changed internal link, CTA, route, anchor, and API target returns successful content or an intentional redirect; external targets have sanitized provider evidence.", category: "functional", source: "cocoframe:target-reachability", required: true, status: "pending" },
    { id: "framework-interaction-integrity", title: "Every changed link and control has an accessible name, keyboard behavior, visible focus, and an action matching its label.", category: "accessibility", source: "cocoframe:interaction-integrity", required: true, status: "pending" },
