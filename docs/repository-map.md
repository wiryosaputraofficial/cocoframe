...
| `@cocoframe/qa` | `packages/qa/src/index.ts` and `design.ts`; adaptive QA decisions, Design Profiles, Product Design Quality diagnostics, traceable cases, allow-listed quality gates, defects, evidence, and explicit release approval. | `tests/qa.test.ts`, `tests/design-quality.test.ts` |
...
| `@cocoframe/database-postgres` | `packages/database-postgres/src/index.ts` and `executor.ts`; pool-compatible operations, managed CocoQL execution, transactions, release/cancellation safety, retry, telemetry, and checksum-locked migrations for PostgreSQL 14–18. | `tests/database-postgres.test.ts`, `tests/database-postgres.integration.test.ts` |
