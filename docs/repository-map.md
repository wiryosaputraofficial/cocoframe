...
| `@cocoframe/cli` | `packages/cli/src/main.ts` and `project.ts`; discovery, inspect, dev/start, build, generation, asset serving, and project integration. | `tests/project.test.ts` |
...
UI, client, CLI, and icons are the intentional multi-file exceptions.
